...
import { BaseShipModel, CustomShipConfig, GameMode, SavedShipProfile, TrailerType, hasHelicopterLandingArea } from '../types/ship';
...
export const ShipyardView: React.FC<ShipyardViewProps> = ({
  ...
  const [gameHistoryCount, setGameHistoryCount] = useState<number>(() => getGameHistory().length);
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-100 flex flex-col selection:bg-amber-600 selection:text-stone-950">
      ...
      <header className="border-b border-stone-800 bg-stone-950/90 backdrop-blur-md sticky top-0 z-30 px-4 lg:px-8 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5">
            {/* Battle History Service Record Button */}
            <button
              onClick={() => {
                setGameHistoryCount(getGameHistory().length);
                setIsGameHistoryModalOpen(true);
              }}
              className="px-3 py-2 rounded-xl bg-zinc-800/90 hover:bg-zinc-700 text-zinc-200 border border-zinc-700 hover:border-amber-500/60 transition flex items-center gap-1.5 text-xs font-semibold shadow-sm cursor-pointer"
              title="Review previous games, performance statistics, vehicle loadouts, and maps"
            >
      ...
      {/* Battle History / Service Record Modal */}
      <GameHistoryModal
